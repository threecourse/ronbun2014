...
class EndowRate(BaseRate):
    ...
    def BaseP_Rate(s):
        # monthly rate
        kiso = 0
        numer =( (s.CT(kiso).Mx(s.x) - s.CT(kiso).Mx(s.x + s.n) + s.CT(kiso).Dx(s.x + s.n)) 
                   / s.CT(kiso).Dx(s.x) 
                  + s.Alpha() 
    ...
    def NetV_Rate(s, t, kiso):
        ...
        return (
                   (s.CT(kiso).Mx(s.x + t) - s.CT(kiso).Mx(s.x + s.n) + s.CT(kiso).Dx(s.x + s.n)) 
                    / s.CT(kiso).Dx(s.x + t) 
    ...
    def ZillV_Rate(s, t, kiso, z):
        ...
        if (t < zm): 
            return s.NetV_Rate(t, kiso) \
                 - s.ZillAlpha() * s.CT(kiso).Annuity_xn(s.x + t, zm - t) / s.CT(kiso).Annuity_xn(s.x, zm);
        return s.NetV_Rate(t, kiso);
